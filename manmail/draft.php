...
if ($_SESSION['login']) {
    ...
    if (isset($_POST["delete_all"]) && $_POST["delete_all"] != "") {
        ...
        $result = $obj->deleteall_draft($email);
    }
...
?>
    ...
    <body>
        ...
        <div class="container-fluid">
            <div class="row">
                <nav id="sidebar" class="d-md-block col-lg-2 col-md-3 my-5 sidebar collapse">
                    <div class="position-sticky "style="margin: -7px 0px 0px 82px;">
                        <ul class="nav flex-column">
                            <li class="compose">
                                <a href="dashboard.php"><button class=" btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#staticBackdrop">Compose</button></a>
                            </li>
                                            ...
                                            <table class="table" id="removetable">
                                                <tbody>
                                                    ...
                                                    while ($row = mysqli_fetch_array($sql)) {
                                                        ...
                                                        $_SESSION['id'] = $row['id'];
                                                    ?>
                                                        <tr class="mesaagedetail" detailid="<?php echo $row['id']; ?>">
                                                            <td>
                                                            <td><input type="checkbox" class="checkbox" name=""></td>
                                                            <input type="hidden" name="message_draft" id="message_draft" value="<?php echo $row['id']; ?>">
                                                            </td>
                                                            <td class="col-4" onclick="window.location='draftdetail.php';"><?php echo $row['to_send'] ?></td>
                                                            <td class="col-4" onclick="window.location='draftdetail.php';"><?php echo $row['subject_line'] ?></td>
                                                            <td class="col-4" onclick="window.location='draftdetail.php';"><?php echo $row['date_time'] ?></td>
                                                        </tr>
                                                    <?php
                                                    }
                                                    ?>
...
if (isset($_SESSION['draft_delete_code']) && $_SESSION['draft_delete_code'] != '') {

?>
    <script>
        swal({
            title: "<?php echo $_SESSION['draft_delete_code']  ?>",
            text: "",
            icon: "<?php echo $_SESSION['draft_code']  ?>",
            button: "Ok Done",
        });
...
<?php
    unset($_SESSION['draft_delete_code']);
}
...
if (isset($_SESSION['deleteall_draft']) && $_SESSION['deleteall_draft'] != '') {

?>

    <script>
        swal({
            title: "<?php echo $_SESSION['deleteall_draft']  ?>",
            text: "",
            icon: "<?php echo $_SESSION['delete_draft']  ?>",
            button: "Ok Done",
        });
    </script>
<?php
    unset($_SESSION['deleteall_draft']);
}
...
?>
        ...
        <script>
            $($document).ready(function() {
                $("#select_all").on("click", ".checkbox", function() {
                    if ($("input[ type=checkbox]").is(":checked")) {
