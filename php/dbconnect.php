...
class dbconnection
{
    ...
    {
        $sql = "SELECT * FROM email WHERE to_send='$email' AND (todelete=1 OR fromdelete=1) ";
        $result = $this->connect_db->query($sql);
        return $result;
    }
    ...
    public function pagination_trash($page, $data, $email)
    {
        ...
        $offset = ($page - 1) * $limit_per_page;
        $sql = "SELECT * FROM email WHERE  to_send='$email' AND (todelete=1 OR fromdelete=1)  ORDER BY id DESC LIMIT $offset,$limit_per_page";
        $result = $this->connect_db->query($sql);
        $output = "";
        if (mysqli_num_rows($result) > 0) {
            // $row = mysqli_fetch_assoc($result);
